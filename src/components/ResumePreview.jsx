      ...
      <div className="resume-preview dark-theme">
        ...
        <div className="left-section">
            ...
            <>
              ...
              {data.experiences.map((exp, idx) => (
                <div className="job-section" key={idx}>
                  <p className="job-title">{exp.jobTitle} @ {exp.companyName}</p>
                  <p className="job-duration">{exp.startDate} - {exp.endDate}</p>
          ...
          {(data.schoolName || data.qualification || data.fieldOfStudy) && (
            <>
              <h3>Education</h3>
              <p><strong>{data.qualification}</strong> in {data.fieldOfStudy}</p>
              <p>{data.schoolName}</p>
              ...
              {data.projects.map((project, idx) => (
                <div key={idx}>
                  ...
                  {project.links.length > 0 && (
                    <p>
                      <strong>Links:</strong> {project.links.map((link, i) => (
          ...
          {Array.isArray(data.volunteers) && data.volunteers.length > 0 && (
            <>
              ...
              {data.volunteers.map((entry, idx) => (
                <div key={idx}>
                  <p><strong>{entry.role}</strong> at {entry.organization}</p>
